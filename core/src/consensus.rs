...
pub const GENESIS_BLOCK_REWARD: u64 = 10_000_000_000_000_000;
...
pub fn calc_mwc_block_overage(height: u64, genesis_had_reward: bool) -> u64 {
    ...
    for _x in 0..MWC_GROUPS_NUM {
        ...
        if block_count < MWC_BLOCKS_PER_GROUP { break;}

        block_count -= MWC_BLOCKS_PER_GROUP;

    }

    if !genesis_had_reward { // Deducting the first block reward if it is 0. This case is used into the tests.
...
mod test {
	...
	fn test_calc_mwc_block_reward() {
		...
		assert_eq!( calc_mwc_block_reward(0), 10_000_000*1_000_000_000 ); // group 1, genesis 10M
        ...
        assert_eq!( calc_mwc_block_reward(2_100_000), 1_190_476_190); // group 2
        assert_eq!( calc_mwc_block_reward(MWC_BLOCKS_PER_GROUP), MWC_FIRST_GROUP_REWARD/2); // group 2
    ...
    fn test_calc_mwc_block_overage() {
        ...
        assert_eq!( calc_mwc_block_overage(1,true), genesis_reward + MWC_FIRST_GROUP_REWARD*1 );

        assert_eq!( calc_mwc_block_overage(30,true), genesis_reward + MWC_FIRST_GROUP_REWARD*30 );
        assert_eq!( calc_mwc_block_overage(30,false), MWC_FIRST_GROUP_REWARD*30 );
        // last block in the first group
        assert_eq!( calc_mwc_block_overage(MWC_BLOCKS_PER_GROUP-1,true), genesis_reward + MWC_FIRST_GROUP_REWARD*(MWC_BLOCKS_PER_GROUP-1) );
        // first block in the second group
        assert_eq!( calc_mwc_block_overage(MWC_BLOCKS_PER_GROUP,true), genesis_reward + MWC_FIRST_GROUP_REWARD*(MWC_BLOCKS_PER_GROUP-1) + MWC_FIRST_GROUP_REWARD/2 );

        assert_eq!( calc_mwc_block_overage(MWC_BLOCKS_PER_GROUP + 5000,true), genesis_reward + MWC_FIRST_GROUP_REWARD*(MWC_BLOCKS_PER_GROUP-1) +
                5001 * MWC_FIRST_GROUP_REWARD/2 );

        // Calculating the total number of coins
        let total_blocks_reward = calc_mwc_block_overage(2_100_000_000*320,true);
        // Expected 10M in total. The coin base is almost 20M
        assert_eq!( total_blocks_reward, 19_999_997_577_247_620);
    }
}
