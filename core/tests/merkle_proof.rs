...
fn pmmr_merkle_proof() {
	...
	assert_eq!(
		proof.path,
		vec![pos_1, pos_5, (pos_9, pos_10).hash_with_index(11).unwrap()]
	);
	assert!(proof.verify(pmmr.root().unwrap(), &elems[0], 0).is_ok());
	...
	assert_eq!(
		proof.path,
		vec![pos_0, pos_5, (pos_9, pos_10).hash_with_index(11).unwrap()]
	);
	assert!(proof.verify(pmmr.root().unwrap(), &elems[1], 1).is_ok());
	...
	assert_eq!(
		proof.path,
		vec![pos_4, pos_2, (pos_9, pos_10).hash_with_index(11).unwrap()]
	);
	assert!(proof.verify(pmmr.root().unwrap(), &elems[2], 3).is_ok());
	...
	assert_eq!(
		proof.path,
		vec![pos_3, pos_2, (pos_9, pos_10).hash_with_index(11).unwrap()]
	);
