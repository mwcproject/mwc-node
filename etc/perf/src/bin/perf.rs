...
fn main() -> Result<(), Error> {
	...
	out_vec.push(build::coinbase_input(
		consensus::MWC_FIRST_GROUP_REWARD,
		key_id2.clone(),
	));
	out_vec.push(build::output(
		consensus::MWC_FIRST_GROUP_REWARD - 1000,
		key_id3.clone(),
	));
	for i in 0..1000 {
		out_vec.push(build::output(
	...
	let tx1 = build::transaction(KernelFeatures::Plain { fee: 0 }, &out_vec[..], &kc, &pb).unwrap();
	...
	let tx2 = build::transaction(KernelFeatures::Plain { fee: 0 }, &out_vec[..], &kc, &pb).unwrap();

	out_vec[2] = build::output(1, ExtKeychainPath::new(1, 3001, 0, 0, 0).to_identifier());
	print("building tx3".to_string());
	let tx3 = build::transaction(KernelFeatures::Plain { fee: 0 }, &out_vec[..], &kc, &pb).unwrap();

	out_vec[2] = build::output(1, ExtKeychainPath::new(1, 3002, 0, 0, 0).to_identifier());
	print("building tx4".to_string());
	let tx4 = build::transaction(KernelFeatures::Plain { fee: 0 }, &out_vec[..], &kc, &pb).unwrap();

	out_vec[2] = build::output(1, ExtKeychainPath::new(1, 3003, 0, 0, 0).to_identifier());
	print("building tx5".to_string());
	let tx5 = build::transaction(KernelFeatures::Plain { fee: 0 }, &out_vec[..], &kc, &pb).unwrap();

	print("start validate tx".to_string());
	let now = std::time::SystemTime::now();
	tx1.validate(Weighting::AsTransaction, verifier_cache.clone())?;
	tx2.validate(Weighting::AsTransaction, verifier_cache.clone())?;
	tx3.validate(Weighting::AsTransaction, verifier_cache.clone())?;
	tx4.validate(Weighting::AsTransaction, verifier_cache.clone())?;
	tx5.validate(Weighting::AsTransaction, verifier_cache.clone())?;
