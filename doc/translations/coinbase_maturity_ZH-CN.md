...
> **当前实现说明：** 下文部分内容描述了早期设计，其中输入会提供针对
> coinbase 输出创建区块的默克尔证明。当前节点不使用起源区块头证明来验证
> coinbase 期限，而是根据当前 UTXO 位置和规范链的截止区块头进行验证。API
> 返回的默克尔证明只针对当前 output PMMR 状态。压缩只保留当前状态所需的最大
> 已修剪子树根，并不保留每个历史 MMR 大小的所有峰。因此，验证证明时必须使用
> 与证明内 `mmr_size` 相匹配的 output root，而不能直接使用输出的起源区块头。
