...
> **현재 구현 참고:** 아래 내용 중 일부는 입력이 코인베이스 출력을 만든
> 블록에 대한 Merkle proof를 제공하던 초기 설계를 설명합니다. 현재 노드는
> 코인베이스 만기 검증에 생성 블록의 과거 proof를 사용하지 않습니다. 만기는
> 현재 UTXO 위치와 정규 체인의 cutoff 헤더를 사용해 검증합니다. API가 반환하는
> Merkle proof는 현재 output PMMR 상태에 대해서만 유효합니다. Compaction은 현재
> 상태에 필요한 최대 pruned-subtree root만 보존하며 모든 과거 MMR 크기의 peak를
> 보존하지 않습니다. 따라서 proof는 출력 생성 블록 헤더가 아니라 proof에 포함된
> `mmr_size`와 일치하는 output root로 검증해야 합니다.
