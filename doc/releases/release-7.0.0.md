...
- define output Merkle proofs as current-PMMR-state proofs for unspent outputs, remove the origin-header argument and historical-size proof path, and document that callers must match the proof's `mmr_size` to the verification root.
...
- keep proof generation on the current output PMMR size and explicitly leave arbitrary historical proofs unsupported because compaction does not retain every historical peak.
...
- update tests for context-aware output deserialization, add coverage for invalid Merkle proofs and invalid commitments, verify short range proofs still parse while oversized proofs are rejected, and add a coinbase Merkle-proof regression confirming that current-state proof generation does not require an origin header.
