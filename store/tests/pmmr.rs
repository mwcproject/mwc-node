...
fn pmmr_leaf_idx_iter() {
	...
	{
		let mut backend =
			store::pmmr::PMMRBackend::new(data_dir.to_string(), true, ProtocolVersion(1), 0, None)
				.unwrap();
...
fn pmmr_append() {
	...
	{
		let mut backend =
			store::pmmr::PMMRBackend::new(data_dir.to_string(), false, ProtocolVersion(1), 0, None)
				.unwrap();
...
fn pmmr_rewind() {
	...
	{
		let mut backend =
			store::pmmr::PMMRBackend::new(data_dir.clone(), true, ProtocolVersion(1), 0, None)
				.unwrap();
