...
/// Poll often enough to notice child exit without delaying the next bridge attempt.
...
pub(crate) mod sealed {
	...
	/// Own a PT process independently of its blocking stdout reader.
	///
	/// In particular, shutdown must not depend on the child producing another
	/// stdout line: a successfully initialized PT is commonly silent while it
	/// waits for SOCKS connections. The explicit shutdown channel lets dropping
	/// [`AsyncPtChild`] close stdin immediately, then kill and reap a child that
	/// does not honor `TOR_PT_EXIT_ON_STDIN_CLOSE` within the grace period.
	fn supervise_pt_child(
		mut child: Child,
		stdin: ChildStdin,
		shutdown: std_mpsc::Receiver<()>,
		ident: String,
	) {
		// Observe both normal child exit and an explicit owner shutdown. A
		// blocking Child::wait() cannot be used here because it would prevent
		// the shutdown channel from waking this supervisor.
		loop {
			match child.try_wait() {
				Ok(Some(status)) => {
					debug!("PT {} exited with status {}.", ident, status);
					return;
				}
				Ok(None) => {}
				Err(e) => {
			...
			match shutdown.recv_timeout(CHILD_STATUS_POLL_INTERVAL) {
		...
		// Managed transports are started with TOR_PT_EXIT_ON_STDIN_CLOSE=1.
		// webtunnelclient responds to this EOF by stopping its listener and
		// terminating, so stdin must be owned outside the blocking stdout reader.
		drop(stdin);

		let deadline = Instant::get() + GRACEFUL_EXIT_TIME;
		loop {
			match child.try_wait() {
				Ok(Some(status)) => {
					debug!(
						"PT {} shut down successfully with status {}.",
						ident, status
					);
					return;
				}
			...
			let remaining = deadline.saturating_duration_since(Instant::get());
			if remaining.is_zero() {
				break;
			}
			thread::sleep(CHILD_STATUS_POLL_INTERVAL.min(remaining));
		}

		debug!("Sending kill signal to PT {}", ident);
		if let Err(e) = child.kill() {
			// The child can exit between the last try_wait() and kill(). Check
			// once more before reporting a failed forced shutdown.
			match child.try_wait() {
				Ok(Some(status)) => {
					debug!("PT {} exited with status {} before kill().", ident, status);
				}
				_ => warn_report!(e, "Failed to kill() spawned PT {}", ident),
			}
			return;
		}

		// Child::kill() does not reap the process. Waiting here prevents a
		// terminated PT from remaining as a zombie on Unix.
		match child.wait() {
			Ok(status) => debug!("PT {} was killed and reaped with status {}.", ident, status),
			Err(e) => warn_report!(e, "Failed to wait() for killed PT {}", ident),
		}
	}
	...
	pub struct AsyncPtChild {
		...
		/// Wake the process supervisor without waiting for child stdout.
		shutdown: std_mpsc::Sender<()>,
		/// Joined on drop so a failed PT is gone before another one is started.
	...
	impl AsyncPtChild {
		...
		pub fn new(mut child: Child, identifier: String) -> Result<Self, PtError> {
			...
			let reader_ident = identifier.clone();
			// Keep Child and ChildStdin out of this closure. A functioning PT is
			// normally silent after negotiation, so this blocking stdout read
			// cannot be relied upon to notice that AsyncPtChild was dropped.
			thread::spawn(move || {
				...
				for line in reader.lines() {
					let err = line.is_err();
					match &line {
						Ok(l) => trace!("<-- PT {}: {:?}", reader_ident, l),
						Err(e) => trace!("<-- PT {}: Error: {:?}", reader_ident, e),
					}
					if let Err(e) = tx.try_send(line) {
						if e.is_disconnected() {
							debug!("PT {} output receiver is disconnected.", reader_ident);
							break;
						}
			...
			let supervisor_ident = identifier.clone();
			// The separate owner can be woken explicitly even while the stdout
			// thread is blocked, allowing failed bridge retries to close the old
			// transport before starting a new webtunnelclient.
			let supervisor = thread::spawn(move || {
				supervise_pt_child(child, stdin, shutdown_rx, supervisor_ident)
			});
	...
	impl Drop for AsyncPtChild {
		fn drop(&mut self) {
			// Arti drops this handle when it discards a failed bridge. Wake the
			// process owner directly; waiting for another stdout line caused
			// quiet webtunnelclient instances to survive into later retries.
			// The supervisor may already have reaped a normally exited child,
			// in which case send() harmlessly returns an error.
			let _ = self.shutdown.send(());
			// Joining is intentional: when Drop returns, the previous bridge's
			// process has exited or has gone through the bounded kill path, so
			// the next attempt cannot accumulate another live PT instance.
			if let Some(supervisor) = self.supervisor.take() {
				if supervisor.join().is_err() {
					warn!("PT {} process supervisor panicked", self.identifier);
				}
			}
		}
	}
...
mod test {
	...
	use crate::ipc::{sealed::AsyncPtChild, PtMessage, PtStatus};
	use std::borrow::Cow;
	use std::collections::HashMap;
	use std::fs;
	use std::io::Read;
	...
	const QUIET_CHILD_READY_ENV: &str = "TOR_PTMGR_TEST_QUIET_CHILD_READY";
	const QUIET_CHILD_STOPPED_ENV: &str = "TOR_PTMGR_TEST_QUIET_CHILD_STOPPED";
	static QUIET_CHILD_TEST_ID: AtomicU64 = AtomicU64::new(0);

	/// Subprocess helper for `dropping_quiet_child_stops_and_reaps_process`.
	///
	/// The regular test invocation has no marker environment and returns. The
	/// spawned invocation remains silent and waits solely for stdin EOF, which
	/// reproduces webtunnelclient after its initial PT negotiation completes.
	#[test]
	fn quiet_pt_child_helper() {
		let Some(ready_path) = std::env::var_os(QUIET_CHILD_READY_ENV) else {
			return;
		};
		let stopped_path = std::env::var_os(QUIET_CHILD_STOPPED_ENV)
			.expect("quiet PT child missing stopped marker path");

		fs::write(ready_path, b"ready").unwrap();
		let mut input = Vec::new();
		std::io::stdin().read_to_end(&mut input).unwrap();
		fs::write(stopped_path, b"stopped").unwrap();
	}

	fn wait_for_test_marker(path: &std::path::Path) {
		let deadline = StdInstant::now() + StdDuration::from_secs(5);
		while !path.exists() && StdInstant::now() < deadline {
	...
	#[test]
	fn dropping_quiet_child_stops_and_reaps_process() {
		let id = QUIET_CHILD_TEST_ID.fetch_add(1, Ordering::Relaxed);
		let marker_prefix = format!("tor-ptmgr-quiet-child-{}-{id}", std::process::id());
		let ready_path = std::env::temp_dir().join(format!("{marker_prefix}-ready"));
		let stopped_path = std::env::temp_dir().join(format!("{marker_prefix}-stopped"));

		let child = Command::new(std::env::current_exe().unwrap())
			.args(["--exact", "ipc::test::quiet_pt_child_helper", "--nocapture"])
			.env(QUIET_CHILD_READY_ENV, &ready_path)
			.env(QUIET_CHILD_STOPPED_ENV, &stopped_path)
			.stdin(Stdio::piped())
			.stdout(Stdio::piped())
			.stderr(Stdio::null())
			.spawn()
		...
		let child = AsyncPtChild::new(child, "quiet-test-pt".into()).unwrap();
		wait_for_test_marker(&ready_path);
		// Let the stdout worker consume the test harness's startup output and
		// block on the now-quiet child before disconnecting its receiver.
		thread::sleep(StdDuration::from_millis(100));

		drop(child);

		// Drop joins the supervisor, so the EOF-aware helper must have exited
		// and the child must have been reaped before control returns here.
