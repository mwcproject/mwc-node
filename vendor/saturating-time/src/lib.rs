...
pub trait SaturatingTime: internal::SaturatingTime {
	...
	///
	/// // Adding 1s to the maximum value will fail.
	/// assert!(max.checked_add(Duration::new(1, 0)).is_none());
	///
	/// // Subtracting 1s from the maximum value will work of course.
	/// assert!(max.checked_sub(Duration::new(1, 0)).is_some());
	/// ```
	fn max_value() -> Self {
	...
	///
	/// // Subtracting 1s from the minimum value will fail.
	/// assert!(min.checked_sub(Duration::new(1, 0)).is_none());
	///
	/// // Adding 1s to the minimum value will work of course.
	/// assert!(min.checked_add(Duration::new(1, 0)).is_some());
	/// ```
	fn min_value() -> Self {
	...
	///
	/// // Adding 1s would overflow so we saturate to the maximum.
	...
	///
	/// // Subtracting 1s would overflow so we saturate to the minimum.
...
mod tests {
	...
	fn minimum_effective_step<T: SaturatingTime + PartialEq>() -> Duration {
		let anchor = <T as internal::SaturatingTime>::anchor();
		let mut nanos = 1;
		loop {
			let step = Duration::from_nanos(nanos);
			if <T as internal::SaturatingTime>::checked_add(&anchor, step) != Some(anchor) {
				return step;
			}
			nanos *= 10;
		}
	...
	>() {
		let step = minimum_effective_step::<T>();
		let max = <T as SaturatingTime>::max_value();
		assert_eq!(max.saturating_add(Duration::ZERO), max);
		assert_eq!(max.saturating_add(step), max);
		assert_eq!(max.saturating_sub(Duration::ZERO), max);
		assert_eq!(max.saturating_sub(step), max - step);

		let min = <T as SaturatingTime>::min_value();
		assert_eq!(min.saturating_sub(Duration::ZERO), min);
		assert_eq!(min.saturating_sub(step), min);
		assert_eq!(min.saturating_add(Duration::ZERO), min);
		assert_eq!(min.saturating_add(step), min + step);
	}
	...
	fn saturating_duration<T: SaturatingTime + PartialEq + Debug>() {
		...
		assert_ne!(max.saturating_duration_since(min), Duration::ZERO);
