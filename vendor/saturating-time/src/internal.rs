...
/// [`None`] and the [`Duration`] has reached 1ns, or the platform rounds the
/// operation to no change because the step is below its clock precision.
///
/// # Algorithm
///
/// 1. Set `step` to `INITIAL_STEP` and `res` to [`SaturatingTime::anchor()`].
/// 2. Call `f(&res, step)`.
///     1. If [`Some`] returns a different value, set `res` to it and continue.
///     2. If [`Some`] returns `res`, return `res`.
///     3. If [`None`] and `step == 1ns`, return `res`.
///     4. Else, set `step` to `MAX{1ns, step / 2}` and continue.
	...
	F: Fn(&T, Duration) -> Option<T>,
{
	...
	loop {
		...
		match next {
			Some(st) => {
				// Windows SystemTime uses 100ns intervals. A smaller duration
				// succeeds without changing the value, so it marks the limit.
				if st == res {
					return res;
				}
...
mod tests {
	...
	fn minimum_effective_step<T: SaturatingTime>() -> Duration {
		let anchor = T::anchor();
		let mut nanos = 1;
		loop {
			let step = Duration::from_nanos(nanos);
			if anchor.checked_add(step) != Some(anchor) {
				return step;
			}
			nanos *= 10;
		}
			...
			+ Sub<Duration, Output = T>,
	{
		...
		assert_eq!(T::max_value().checked_add(step), None);
		assert_eq!(
			T::max_value().checked_sub(Duration::ZERO),
			Some(T::max_value())
		);
		assert_eq!(
			T::max_value().checked_sub(step),
			Some(T::max_value() - step)
		);
		...
		assert_eq!(T::min_value().checked_sub(step), None);
		assert_eq!(
			T::min_value().checked_add(Duration::ZERO),
			Some(T::min_value())
		);
		assert_eq!(
			T::min_value().checked_add(step),
			Some(T::min_value() + step)
		);
	}
