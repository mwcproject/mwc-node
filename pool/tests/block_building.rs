...
fn test_transaction_pool_block_building() -> Result<(), PoolError> {
	...
	let root_tx_1 = test_transaction(&mut secp, &keychain, vec![100, 200], vec![240]);
	let root_tx_2 = test_transaction(&mut secp, &keychain, vec![300], vec![270]);
	let root_tx_3 = test_transaction(&mut secp, &keychain, vec![400], vec![370]);

	let child_tx_1 = test_transaction(&mut secp, &keychain, vec![240], vec![210]);
	let child_tx_2 = test_transaction(&mut secp, &keychain, vec![370], vec![320]);
	...
	let txs = pool.prepare_mineable_transactions(&mut secp)?;
